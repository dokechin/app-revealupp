package App::revealupp::cli::serve;
...
sub run {
    ...
    if( !$html ) {
        system "perldoc App::revealupp::cli::serve";
        exit;
    }
...
sub app {
    ...
    return sub {
        ...
        if($self->theme_path && $env->{PATH_INFO} =~ m!${self->theme_path}$!){
            ...
            return App::revealupp::util::path_to_res($path) if $path->exists;
        }
        
        $path = path('.', $env->{PATH_INFO});
        return App::revealupp::util::path_to_res($path) if $path->exists;

        my $reveal_dir = App::revealupp::util::share_path([qw/share/]);
        $path = $reveal_dir->child($env->{PATH_INFO});
        return App::revealupp::util::path_to_res($path) if $path->exists;
        App::revealupp::util::warn("$path does not exist");
        return [
            404,
...
=head1 SYNOPSIS
    ...
    $ revealupp serve -p 5000 markdown.md

=head1 DESCRIPTION

C<serve> command makes your Markdown text as a HTTP Web application for the slide show.
Run C<revealupp serve> command with the Markdown formatted text file name and options, 
