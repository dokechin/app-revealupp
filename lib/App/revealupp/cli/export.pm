package App::revealupp::cli::export;
...
sub run {
    ...
    if( grep { $_ eq $sub_command } @{$self->sub_commands} ) {
        my $klass = sprintf("App::revealupp::cli::export::%s", lc($sub_command));
...
sub usage {
    system "perldoc App::revealupp::cli::export";
    exit;
}
...
=head1 SYNOPSIS
    ...
    $ revealupp export html --output slides.html
...
=head1 SUB COMMAND: C<theme>
    ...
    $ revealupp export theme --base blood.css --output theme.css
...
=head1 SUB COMMAND: C<html>
    ...
    $ revealupp export html slides.md --theme ./origina.css --output slides.html
